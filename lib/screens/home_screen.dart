...
import 'package:my_emotion/screens/diary_screen.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _onDaySelected(DateTime selectedDay, DateTime focusedDay) async {
    if (selectedDay.isAfter(DateTime.now())) {
      ...
      final diaries = await DiaryService().getDiaries(selectedDay);
      if (diaries.isEmpty) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => DiaryCreationScreen(
              onSave: (content) async {
                await DiaryService().createDiary(content, selectedDay);
                  ...
                  TableCalendar(
                    ...
                    calendarBuilders: CalendarBuilders(
                            ...
                            children: [
                              Align(
                                ...
                                child: Padding(
                                  ...
                                  child: CircleAvatar(
                                    backgroundImage: AssetImage(
                                        'assets/sample_image.png'), // 이미지 경로
                                    radius: 20.0, // 이미지의 반지름
                                  ),
                                ),
                              ),
                              Positioned(
                                top: 4.0, // 날짜를 셀�� 상단에 배치
                                left: 0,
                                right: 0,
                                child: Center(
                                  child: Text(
                      ...
                      outsideBuilder: (context, date, focusedDay) {
                        return SizedBox.shrink(); // 현재 달에 속하지 않는 날짜를 숨��
                      },
                    ),
